...
public class AcceleratorAuthenticationProvider extends CoreAuthenticationProvider
{
	...
	public Authentication authenticate(final Authentication authentication) throws AuthenticationException
	{
		...
		if (getBruteForceAttackCounter().isAttack(username))
		{
			try
			{
				...
				LOG.debug("setLoginDisabled(true) " + authentication.getName());
			...
			throws AuthenticationException
	{
		...
		LOG.debug("additionalAuthenticationChecks " + authentication.getName());

		if (myStoreCustomerFacade.checkUserStatus(details.getUsername()))
		{
			LOG.debug("Status=true " + authentication.getName());
			throw new BadCredentialsException("Customer has lot authentication failures");
		}
