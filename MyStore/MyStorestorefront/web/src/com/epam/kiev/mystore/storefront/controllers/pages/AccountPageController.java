...
import com.epam.kiev.mystore.storefront.breadcrumb.ResourceBreadcrumbBuilder;
import com.epam.kiev.mystore.storefront.controllers.ControllerConstants;
import com.epam.kiev.mystore.storefront.controllers.util.GlobalMessages;
import com.epam.kiev.mystore.storefront.forms.AddressForm;
import com.epam.kiev.mystore.storefront.forms.OrganizationForm;
import com.epam.kiev.mystore.storefront.forms.UpdateEmailForm;
import com.epam.kiev.mystore.storefront.forms.UpdatePasswordForm;
import com.epam.kiev.mystore.storefront.forms.UpdateProfileForm;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_ORGANIZATIONS_PAGE = REDIRECT_PREFIX + "/my-account/organizations";
	private static final String REDIRECT_TO_ADD_ORGANIZATIONS_PAGE = REDIRECT_PREFIX + "/my-account/add-organization";
	/**
	...
	private static final String ORGANIZATION_ID_PATH_VARIABLE_PATTERN = "{organizationId:.*}";
	...
	public String subscriptions(final Model model) throws CMSItemNotFoundException
	{
		...
		try
		{
			...
			Collections.sort(sortedSubscriptions, new Comparator<SubscriptionData>()
			{
				...
				{
					return subscription1.getSubscriptionStatus().compareToIgnoreCase(subscription2.getSubscriptionStatus());
				}
			});
		}
	...
	public String organizations(final Model model) throws CMSItemNotFoundException
	{
		final List<OrganizationData> organizationsData = myStoreCustomerFacade.getOrganizations();

		storeCmsPageInModel(model, getContentPageForLabelOrId(ACCOUNT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ACCOUNT_CMS_PAGE));
		model.addAttribute("organizations", organizationsData);
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.organizations"));
		model.addAttribute("metaRobots", "no-index,no-follow");
		return ControllerConstants.Views.Pages.Account.AccountOrganizationsPage;
	}

	@RequestMapping(value = "/add-organization", method = RequestMethod.GET)
	public String addOrganizations(final Model model) throws CMSItemNotFoundException
	{
		final List<OrganizationData> organizationsData = myStoreCustomerFacade.getNotMyOrganizations();

		storeCmsPageInModel(model, getContentPageForLabelOrId(ACCOUNT_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(ACCOUNT_CMS_PAGE));
		model.addAttribute("organizations", organizationsData);
		model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.organizations.addOrganizations"));
		model.addAttribute("metaRobots", "no-index,no-follow");
		return ControllerConstants.Views.Pages.Account.AccountAddOrganizationPage;
	}

	@RequestMapping(value = "/add-organization/" + ORGANIZATION_ID_PATH_VARIABLE_PATTERN, method = RequestMethod.GET)
	public String addOrganization(@PathVariable("organizationId") final String organizationId,
			final RedirectAttributes redirectModel)
	{
		final OrganizationData organizationData = new OrganizationData();
		organizationData.setId(organizationId);
		myStoreCustomerFacade.addOrganization(organizationData);

		redirectModel.addFlashAttribute(GlobalMessages.CONF_MESSAGES_HOLDER,
				Collections.singletonList("account.confirmation.organization.added"));
		return REDIRECT_TO_ADD_ORGANIZATIONS_PAGE;
	}

	@RequestMapping(value = "/remove-organization/" + ORGANIZATION_ID_PATH_VARIABLE_PATTERN, method = RequestMethod.GET)
	public String removeOrganization(@PathVariable("organizationId") final String organizationId,
			final RedirectAttributes redirectModel)
	{
		final List<OrganizationData> myOrganizationsData = myStoreCustomerFacade.getOrganizations();
		if (myOrganizationsData.size() > 1)
		{
			final OrganizationData organizationData = new OrganizationData();
			organizationData.setId(organizationId);
			myStoreCustomerFacade.removeOrganization(organizationData);

			redirectModel.addFlashAttribute(GlobalMessages.CONF_MESSAGES_HOLDER,
					Collections.singletonList("account.confirmation.organization.removed"));
		}
		else
		{
			redirectModel.addFlashAttribute(GlobalMessages.ERROR_MESSAGES_HOLDER,
					Collections.singletonList("account.confirmation.organization.notRemoved"));
		}
		return REDIRECT_TO_ORGANIZATIONS_PAGE;
	}

	@RequestMapping(value = "/create-organization", method = RequestMethod.GET)
	public String createOrganization(final Model model) throws CMSItemNotFoundException
	{
		...
		return ControllerConstants.Views.Pages.Account.AccountCreateOrganizationPage;
	}

	@RequestMapping(value = "/create-organization", method = RequestMethod.POST)
	public String createNewOrganization(@Valid final OrganizationForm organizationForm, final BindingResult bindingResult,
			final Model model) throws CMSItemNotFoundException
	{
		...
		else
		{
			...
			{
				final OrganizationData organizationData = new OrganizationData();
				organizationData.setId(organizationForm.getId());
				organizationData.setName(organizationForm.getName());
				organizationData.setPhoneNumber(organizationForm.getPhoneNumber());
				organizationFacade.createOrganization(organizationData);
			}
			catch (final DuplicateIdException e)
			{
				GlobalMessages.addErrorMessage(model, "text.account.organizations.id.duplicate");
				storeCmsPageInModel(model, getContentPageForLabelOrId(PROFILE_CMS_PAGE));
				setUpMetaDataForContentPage(model, getContentPageForLabelOrId(PROFILE_CMS_PAGE));
				model.addAttribute("breadcrumbs", accountBreadcrumbBuilder.getBreadcrumbs("text.account.profile"));
				return ControllerConstants.Views.Pages.Account.AccountCreateOrganizationPage;
	...
	@RequestMapping(value = "/edit-organization/" + ORGANIZATION_ID_PATH_VARIABLE_PATTERN, method = RequestMethod.GET)
	public String editOrganizationForm(@PathVariable("organizationId") final String organizationId, final Model model,
			final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		final OrganizationForm organizationForm = new OrganizationForm();
		final OrganizationData organizationData = organizationFacade.getOrganization(organizationId);
		...
		return ControllerConstants.Views.Pages.Account.AccountUpdateOrganizationPage;
	}

	@RequestMapping(value = "/edit-organization/edit", method = RequestMethod.POST)
	public String editOrganization(@Valid final OrganizationForm organizationForm, final BindingResult bindingResult,
			final Model model) throws CMSItemNotFoundException
	{
		if (bindingResult.hasErrors())
		{
		...
		else
		{
			...
			organizationData.setName(organizationForm.getName());
			organizationData.setPhoneNumber(organizationForm.getPhoneNumber());
			organizationFacade.updateOrganization(organizationData);
		}
		return REDIRECT_TO_ORGANIZATIONS_PAGE;
	}
}
