...
import com.epam.kiev.mystore.storefront.security.AutoLoginStrategy;
...
public abstract class AbstractRegisterPageController extends AbstractPageController
{
			...
			final Model model, final HttpServletRequest request, final HttpServletResponse response) throws CMSItemNotFoundException
	{
		...
		final MyStoreRegisterData data = new MyStoreRegisterData();
		data.setFirstName(form.getFirstName());
		data.setLastName(form.getLastName());
		data.setLogin(form.getEmail());
		data.setPassword(form.getPwd());
		data.setTitleCode(form.getTitleCode());
		data.setOrganizationIds(form.getOrganizations());
		try
		{
			myStoreCustomerFacade.register(data);
